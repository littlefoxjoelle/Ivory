...
$result = mysqli_query($link,"SELECT * FROM `товары` INNER JOIN `Категории`ON `товары`.`Код категории` = `категории`.`Код категории` WHERE `Название` LIKE '%$search%' AND `Видимость`='0' $query_brand $query_price ORDER BY `Код платья` ASC");
